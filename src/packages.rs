...
mod tests {
    ...
    fn table(pairs: &[(&str, Entry)]) -> PackageTable {
        PackageTable {
            entries: pairs
                .iter()
    ...
    fn rendering_is_stable_and_sorted_regardless_of_insertion_order() {
        let a = table(&[("b@1.0.0", entry("u2")), ("a@1.0.0", entry("u1"))]);
        let b = table(&[("a@1.0.0", entry("u1")), ("b@1.0.0", entry("u2"))]);
    ...
    fn the_generated_header_and_version_lead_the_file() {
        let text = table(&[("a@1.0.0", entry("u"))]).render();
        assert!(
    ...
    fn a_bin_map_and_a_true_flag_are_rendered_inline() {
        ...
        let text = table(&[("a@1.0.0", e)]).render();
        assert!(
    ...
    fn bin_names_that_are_not_bare_toml_keys_are_quoted() {
        ...
        let Loaded::Present(parsed) = load(&path).unwrap() else {
            panic!("expected a parsed package table");
        };
    ...
    fn render_then_load_round_trips() {
        ...
        e.has_install_script = true;
        let original = table(&[("@scope/a@1.0.0", e), ("b@2.0.0", entry("u2"))]);
    ...
    fn a_root_that_is_not_package_round_trips_through_render_and_load() {
        ...
        let original = table(&[("@types/node@22.20.0", e)]);
        let text = original.render();
        assert!(
    ...
    #[test]
    fn a_matching_table_is_not_stale() {
        let e = expected(&[("a@1.0.0", "u1", "sha512-AAAA")]);
        let s = Loaded::Present(table(&[("a@1.0.0", entry("u1"))]));
    ...
    fn an_entry_for_a_package_no_longer_in_the_graph_is_extra() {
        let e = expected(&[]);
        let s = Loaded::Present(table(&[("gone@1.0.0", entry("u"))]));
        assert_eq!(
            staleness(&e, &s),
    ...
    #[test]
    fn an_absent_table_reports_every_package_as_missing() {
    ...
    fn a_wrong_version_is_a_single_difference_not_a_flood() {
        ...
        assert_eq!(
            ...
            "a package table pudu cannot read is one problem, not one per package"
        );
    }
