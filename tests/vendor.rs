...
impl Fixture {
    ...
    fn with_scopes(scopes: &[(&str, &str)]) -> Self {
        ...
        // Two platforms, deliberately. `packages.toml` is meant to be the union
    ...
    fn table(&self) -> String {
        std::fs::read_to_string(self.table_path()).unwrap()
    }
}

#[test]
fn vendor_writes_a_table_covering_every_package() {
    let f = Fixture::new();
...
fn the_vendored_set_is_the_union_across_platforms_not_one_platform_s_view() {
    ...
    // "`packages.toml` is a function of `pudu.toml`".
    let f = Fixture::new();
    f.cmd().arg("vendor").assert().success();
...
#[test]
fn dropping_a_platform_makes_the_table_stale() {
...
#[test]
fn a_second_run_reproduces_the_table_byte_for_byte() {
    let f = Fixture::new();
    f.cmd().arg("vendor").assert().success();
    let first = f.table();
    f.cmd().arg("vendor").assert().success();
    assert_eq!(first, f.table());
}

#[test]
fn a_re_download_from_a_cold_cache_reproduces_the_table_byte_for_byte() {
    // `a_second_run_reproduces_the_table_byte_for_byte` carries every entry
    // over and downloads nothing, so it never exercises the parallel path.
    // Deleting the table *and* the cache forces a genuine concurrent
    ...
    std::fs::remove_file(f.table_path()).unwrap();
    ...
    assert_eq!(
        first,
        f.table(),
        "a parallel re-download must reproduce the table byte for byte"
    );
}
...
fn check_exits_five_and_names_the_package_when_stale() {
    ...
    std::fs::write(f.table_path(), &text[..cut]).unwrap();
...
#[test]
fn check_exits_five_when_the_table_is_absent() {
    let f = Fixture::new();
    let out = f.cmd().args(["vendor", "--check"]).output().unwrap();
    assert_eq!(out.status.code(), Some(5));
}
...
#[test]
fn a_corrupt_table_reports_the_underlying_parse_error() {
    // "cannot read packages.toml" on its own leaves the user guessing. The
...
fn a_tarball_whose_bytes_fail_the_integrity_aborts_naming_the_package() {
    ...
    assert!(
        ...
        "a failed run must not write a table"
    );
}
...
#[test]
fn a_config_with_no_platforms_refuses_rather_than_writing_an_empty_table() {
    let f = Fixture::new();
    std::fs::write(
        f.dir.path().join("pudu.toml"),
        format!(
